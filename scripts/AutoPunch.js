...
if (startApp(packageName)) {
    ...
    if (workbench) {
        ...
        let isSuccess = false;
        for (let i = 0; i < 3; i++) {
            console.log(`第${i + 1}次检查工作台标题...`);
            // 使用更精确的选择器
            let workbenchTitle = textMatches("工作台").className("android.widget.TextView")
                .depth(2).clickable(true).findOne(2000);

            if (workbenchTitle) {
                let bounds = workbenchTitle.bounds();
                console.log("找到标题元素：", workbenchTitle.text(),
                    "位置：", bounds,
                    "深度：", workbenchTitle.depth(),
                    "可点击：", workbenchTitle.clickable());

                // 验证位置是否在顶部区域
                if (bounds.top >= 124 && bounds.top <= 188 &&
                    bounds.left >= 469 && bounds.right <= 610) {
                    isSuccess = true;
                    console.log("工作台切换成功！标题位置匹配！");
                    toast("已切换到工作台");
                    ...
                    console.log("找到工作台文字但位置不匹配");
                }
            } else {
                console.log("未找到符合条件的工作台标题元素");
            }
            sleep(1000);
        }

        if (!isSuccess) {
            console.log("首次切换失败，尝试重试...");
            workbench.parent().click();
            sleep(2000);
            console.log("重试后检查工作台标题...");
            workbenchTitle = id("mry").findOne(2000);
            if (workbenchTitle && workbenchTitle.text() === "工作台" && workbenchTitle.bounds.top < 200) {
                console.log("重试后切换成功！");
                toast("重试后成功切换到工作台");
            } else {
                console.log("重试后仍未切换成功");
            }
        }
    } else {
